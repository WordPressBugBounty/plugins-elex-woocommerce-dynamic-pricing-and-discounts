...
function elex_dp_get_product_tag_by_id( $tag_id) {
...
function eh_product_search_select2_field( $name = 'product_id', $style = 'width:100%;', $tab = 'product_rules', $multiple = true) {
	if (elex_dp_is_wc_version_gt_eql('2.7')) {
		...
		?>
		<input type="hidden" class="wc-product-search" data-multiple="true"  name="<?php echo $name; ?>" data-placeholder="<?php esc_attr_e('Search for a product&hellip;', 'eh-dynamic-pricing-discounts'); ?>" data-action="woocommerce_json_search_products_and_variations" data-selected="
																							  <?php
			   $json_ids = array();
																								if (isset($_GET['edit'])) {
																									$rules = get_option('xa_dp_rules');
...
function elex_dp_init_wc_functions() {
	...
	if (!function_exists('xa_get_tag_ids')) {

		function xa_get_tag_ids( $product) { 
			$prod_id =$product;
			if (is_object($product)) {
				$prod_id = elex_dp_get_pid($product);
			} else {
			...
			if ($terms) {
				foreach ($terms as $key => $term) {
					array_push($tags_ids_array, $term->term_id);
					$term2 = $term;

					if (!in_array($term2->parent, $tags_ids_array)) {
						while ($term2->parent > 0) {
							array_push($tags_ids_array, $term2->parent);
							$term2 = get_term_by('id', $term2->parent, 'product_tag');
						}
					}
				}
			}
...
return $id;
	...
	function elex_dp_get_attributes_values_selectoptions( $taxonomy, $selected_value = '') {
	   global $wpdb,$at_value_map,$at_taxonomy_map,$at_taxonomy_list,$at_value_options,$at_taxonomy_options;
